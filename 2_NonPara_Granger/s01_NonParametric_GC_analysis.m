...
for gg = 1:size(useCoordPairs,1)
    ...
    savename = ['QH_extract_all_5mm_NoF_' num2str(theseTrials) '_CohPower_GC_' num2str(possCoords{1}) ...
        '_and_' num2str(possCoords{2}) '_0to500ms_5Tapsmo_TopVoxel_FINAL.mat']; %savename
    ...
    for hh = 1:length(possCoords)
        ...
        listCoords = ['Coord_NoF_FINAL_' num2str(possCoords{hh}) '_by_TstatOverTime_0_to_500ms'];
        ...
        for voxel = 1:size(coordList,1)
            ...
            GM_faces(voxel,:,:) = waveform;%(1:151);% feature x time x trial
        end
        ...
        for voxel = 1:size(coordList,1)
            ...
            load ([filepath filename])
            GM_random(voxel,:,:) = waveform;%(1:151);% feature x time x trial
        end
    ...
    for ii = 1:nTrials
        ...
        save(savename, 'allCoherenceInfo','allPowerInfo', 'allGranger', 'mFreq')
