...
clear all; close all;
%Add the SPM12 toolbox and the bsmart GC toolbox to the path
...
nTimePts = 301; %number of timepoints within each trial
...
for gg = 1:length(poss_a)
    ...
    for  ff = 1:length(poss_c)
        ...
        for vv = 1:length(poss_d)
            ...
            %Save name
            savename = ['all' num2str(theseTrials) '_CohPower_GC_SYNTHETIC_a=' num2str(this_a) '_c=' num2str(this_c) '_d=' num2str(this_d) '.mat']
 
            %EXTRACT DATA
            Face_PFC = YF_all; % Face data at PFC (or 'first node', i.e 'YF')
            Face_FG = XF_all; %Face data at FG (or 'second node' i.e. 'XF')
            Random_PFC = YR_all; %Random data at PFC (or 'first node', i.e. 'YR')
            Random_FG =XR_all; %Random data at FG (or 'second' node, i.e. 'XR')
            
            %% Run GC 2 node analysis (calcuated on single-trial basis)
            for ii = 1:nTrials
                
                % Put data into fieldtrip format
                time = linspace(0,600,nTimePts)/1000; %time point indices
